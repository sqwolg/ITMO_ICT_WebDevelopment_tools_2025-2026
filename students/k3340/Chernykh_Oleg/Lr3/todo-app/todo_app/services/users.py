...
from todo_app.schemas import UserModel, UserCreateModel, UserUpdateModel
...
class UserService():
    ...
    async def _exists(user: QuerySet, get_query: bool = False) -> User:
        user_exists = await user.exists() if not get_query else True
        if not get_query:
    ...
    async def get_all() -> list[UserModel]:
        ...
        return [await UserModel.from_tortoise_orm(user) for user in users]
